...
def has_text_inside_margins_with_all_margins(page):
    ...
    for word in text:
        ...
        #     return False
        # instead of the above margins let's add small, 1 or 2 points to get more
        # accurate results
        if (
            x0 < left_margin - 2 or  # Left margin
            x1 > (page_width - right_margin + 2) or  # Right margin
            y0 < bottom_margin - 2 or  # Bottom margin
            y1 > (page_height - top_margin + 2)  # Top margin
        ):
            print(f"Page {page.page_number} has text outside the margins.")
            return False
...
if __name__ == "__main__":
    ...
    with pdfplumber.open(input_file) as pdf:
        for i, page in enumerate(pdf.pages):
            ...
            margines_followed = has_text_inside_margins_with_all_margins(page)
            ...
            if not margines_followed:
                ...
                margin_page = draw_margins_on_page_with_all_margins(page_obj)
                modified_page = overlay_page(page_obj, margin_page)
                output.add_page(modified_page)


            images_inside_margins = has_images_inside_margins_with_all_margins(page)
            ...
            if not images_inside_margins:
                ...
                margin_page = draw_margins_on_page_with_all_margins(page_obj, color=(0, 1, 0))
                modified_page = overlay_page(page_obj, margin_page)
                output.add_page(modified_page)
